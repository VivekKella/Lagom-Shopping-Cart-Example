...
public interface ShoppingCartService extends Service {
	 ...
	 * <p>
	 * Example: curl -H "Content-Type: application/json" -X POST -d '{"itemId": 456,
	 * "quantity": 2}' http://localhost:9000/shoppingcart/123
	 */
	 ...
	 *
	 * Example: curl -H "Content-Type: application/json" -X PATCH -d '{"quantity":
	 * 2}' http://localhost:9000/shoppingcart/123/item/456
	 */
	 ...
	 * This gets published to Kafka.
	 */
	Topic<ShoppingCartEvent> helloEvents();

	@Override
	default Descriptor descriptor() {
		// @formatter:off
		return named("shoppingCart")
				.withCalls(restCall(Method.GET, "/shoppingcart/:id", this::get),
				...
				.withTopics(topic(TOPIC_NAME, this::helloEvents)
						// Kafka partitions messages, messages within the same partition will
						// be delivered in order, to ensure that all messages for the same user
						// go to the same partition (and hence are delivered in order with respect
						// to that user), we configure a partition key strategy that extracts the
						// name as the partition key.
						.withProperty(KafkaProperties.partitionKeyStrategy(), ShoppingCartEvent::getName))
				.withAutoAcl(true);
		// @formatter:on
	}
}
